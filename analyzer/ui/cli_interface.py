...
class InteractiveMenu:
    ...
    def _http_submenu(self) -> Optional[str]:
    ...
    def _pandas_submenu(self) -> Optional[str]:
    ...
    def _get_source_ips(self) -> str:
        ...
        lines = [
            f"  {i}. {addr}: {count}"
            for i, (addr, count) in enumerate(ip.ip_source_counter.most_common(n), 1)
        ]
    ...
    def _get_ttls(self) -> str:
        ...
        lines = [
            f"  {i}. TTL {ttl}: {count}"
            for i, (ttl, count) in enumerate(ip.ttl_histogram.most_common(n), 1)
        ]
    ...
    def _get_udp_ports(self) -> str:
        ...
        lines = [
            f"  {i}. {port}: {count}"
            for i, (port, count) in enumerate(udp.port_flows.most_common(n), 1)
        ]
    ...
    def _get_dns_queries(self) -> str:
        ...
        if stats:
            ...
            result += "\nTop queries:\n" + "\n".join(
                f"  {i}. {q}: {c}" for i, (q, c) in enumerate(top, 1)
            )
        return result
    ...
    def _get_patterns(self) -> str:
        ...
        lines = [
            f"  '{p}': {c}" for p, c in sorted(patterns.items(), key=lambda x: x[1], reverse=True)
        ]
    ...
    def _get_port_scan_info(self) -> str:
        ...
        if not sa:
            ...
            count = (
                report.get("anomalies_detected", {})
                .get("anomaly_types", {})
                .get("possible_port_scan", 0)
            )
        ...
        for ip, ports in sorted(scanners.items(), key=lambda x: -x[1])[:n]:
    ...
    def _pandas_top_talkers(self, pa) -> str:
        ...
        for i, t in enumerate(top, 1):
            lines.append(
                f"  {i:2d}. {t.get('src_ip', 'N/A'):15} : {t.get('total_bytes', 0):,} bytes"
            )
        return "\n".join(lines)
    ...
    def _pandas_protocol_analysis(self, pa) -> str:
        ...
        for proto, stats in protocols.items():
            if isinstance(stats, dict):
                ...
                lines.append(f"  {proto:12} : {count:6} packets ({pct:5.1f}%)")
            else:
