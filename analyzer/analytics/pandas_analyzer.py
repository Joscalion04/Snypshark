...
class PandasAnalyzer(PacketProcessor):
    ...
    def _check_anomalies(self, packet_info: Dict) -> None:
        ...
        if packet_info.get("flags") == "0x0002" and packet_info.get("length", 0) < 100:
            anomalies.append("possible_port_scan")
            self.stats["port_scan_attempts"] += 1

        if (
            packet_info.get("protocol") == "ICMP"
            and packet_info.get("length", 0) > SecurityThresholds.ICMP_LARGE_THRESHOLD
        ):
    ...
    def build_dataframes(self) -> None:
        ...
        if self.connections_data:
            ...
            self.df_connections = (
                ...
                .agg({"packet_count": "sum", "total_bytes": "sum"})
                .reset_index()
            )
    ...
    def _get_overview_stats(self) -> Dict:
        ...
        return {
            ...
            "unique_ips": len(
                set(self.df_packets["src_ip"].tolist() + self.df_packets["dst_ip"].tolist())
            ),
        }
    ...
    def _get_top_talkers(self) -> List[Dict]:
        ...
        top_talkers = (
            self.df_packets.groupby("src_ip")["length"]
            .agg(["sum", "count"])
    ...
    def _get_timeline_analysis(self) -> Dict:
        ...
        timeline = (
            self.df_packets.set_index("timestamp")
